...
try {
    ...
    function pr_rounded_rect($img, $x, $y, $w, $h, $radius, $color){
    ...
    if($middleText !== ""){
        ...
        $maxBoxBottom = $h - $footerH - 55;

        if($boxY + $boxH > $maxBoxBottom){
            $boxY = $maxBoxBottom - $boxH;
        }

        $boxColor = imagecolorallocatealpha($canvas, 245, 245, 245, 20);
        pr_rounded_rect($canvas, $boxX, $boxY, $boxW, $boxH, 45, $boxColor);

        $black = imagecolorallocate($canvas, 22, 22, 22);
        $ty = $boxY + $paddingTop + 4;

        foreach($textLines as $ln){
            imagettftext($canvas, $textSize, 0, $boxX + $paddingX, $ty, $black, $fontRegular, $ln);
            $ty += $lineHeight;
        }
    }
