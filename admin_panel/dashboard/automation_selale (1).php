...
<a href="settings_selale.php"> Ayarlar</a>
...
function selaleAscii(text){
return String(text || "")
.toLocaleLowerCase("tr-TR")
.replace(/ı/g,"i").replace(/ğ/g,"g").replace(/ü/g,"u")
.replace(/ş/g,"s").replace(/ö/g,"o").replace(/ç/g,"c");
}
function selaleIsRelationshipTopic(topic){
const t = selaleAscii(topic);
return ["ask","iliski","evlilik","cift","partner","sevgili","esler","baglanma","baglilik","guven","kiskanclik","aldatma","sadakat","ayrilik","romantik"].some(function(k){ return t.includes(k); });
}
function selaleCleanGeneratedText(text){
text = String(text || "");
const replacements = [
[/Uzm\.\s*Dr\.\s*Özgür\s+Özbebit/giu, "Şelale Özbebit"],
[/Özgür\s+Özbebit/giu, "Şelale Özbebit"],
[/#\s*ÖzgürÖzbebit/giu, ""],
[/#\s*OzgurOzbebit/giu, ""],
[/#\s*PsikiyatriBilgisi/giu, ""],
[/#\s*Psikiyatri/giu, ""],
[/#\s*Psikiyatrist/giu, ""],
[/#\s*RuhSağlığı/giu, ""],
[/#\s*RuhSagligi/giu, ""],
[/#\s*MentalHealth/giu, ""],
[/Psikiyatri pratiğinde/giu, "Aile ve ilişki danışmanlığı çalışmalarında"],
[/psikiyatri pratiğinde/giu, "aile ve ilişki danışmanlığı çalışmalarında"],
[/Psikiyatrist/giu, "Aile Danışmanı"],
[/psikiyatrist/giu, "aile danışmanı"],
[/Psikiyatri/giu, "Aile danışmanlığı"],
[/psikiyatri/giu, "aile danışmanlığı"],
[/ruh sağlığı/giu, "duygusal iyi oluş"],
[/Ruh sağlığı/giu, "Duygusal iyi oluş"],
[/doktor/giu, "uzman"],
[/Doktor/giu, "Uzman"],
[/hekim/giu, "uzman"],
...
[/Hasta/giu, "Danışan"],
[/tedavi/giu, "destek"],
[/tanı/giu, "değerlendirme"]
];
replacements.forEach(function(pair){ text = text.replace(pair[0], pair[1]); });
text = text.replace(/[ \t]+\n/g, "\n").replace(/\n{3,}/g, "\n\n").replace(/[ \t]{2,}/g, " ");
return text.trim();
}
function selaleExtractVisualPrompt(conceptText){
const text = String(conceptText || "");
const match = text.match(/GÖRSEL PROMPT:\s*([\s\S]*?)(?:\n\s*KOMPOZİSYON NOTU:|\n\s*KAÇINILACAKLAR:|$)/i);
if(match && match[1]){ return match[1].trim(); }
return text.trim();
}
function buildSelaleImagePrompt(topic, conceptText){
const cleanConceptPrompt = selaleExtractVisualPrompt(conceptText);
if(selaleIsRelationshipTopic(topic)){
return `RELATIONSHIP_HARD_LOCK
Create one ultra realistic premium editorial lifestyle photograph.
Topic: ${topic}
Main scene: exactly two Turkish adults only, one adult woman and one adult man, shown as romantic partners in a mature relationship.
Emotion: subtle jealousy, emotional distance, insecurity, trust repair, calm communication, desire for reassurance.
Setting: warm neutral modern home or elegant relationship counselling style interior, beige, cream, warm sand and muted sage tones, soft natural window light, realistic faces and hands.
Composition: square 1024x1024, clean upper-left space for later title overlay, calm lower area for footer band, no visible text, no logo, no watermark.
Important: follow this adult couple relationship scene above as the primary instruction.
Incoming concept detail:
${cleanConceptPrompt}`;
}
return `Create one ultra realistic premium editorial lifestyle photograph for Şelale Özbebit family and relationship counselling content.
Topic: ${topic}
Follow this visual concept as the primary instruction:
${cleanConceptPrompt}
Composition: square 1024x1024, clean upper-left space for later title overlay, calm lower area for footer band, no visible text, no logo, no watermark.`;
}
...
if(data.success){
box.innerText = selaleCleanGeneratedText(data.content || data.text || data.result || JSON.stringify(data,null,2));
}else{
...
if(data.success){
box.innerText = selaleCleanGeneratedText(data.copy || data.social_copy || data.text || data.caption || JSON.stringify(data,null,2));
}else{
...
box.innerText = selaleCleanGeneratedText(
`Ana Başlık:
${data.main_title || ""}
...
try{
const response = await fetch("../api/generate_visual_concept_selale.php",{
method:"POST",
headers:{"Content-Type":"application/json"},
...
const finalPrompt = buildSelaleImagePrompt(topic, visualPrompt);

try{
const response = await fetch("../api/generate_image_selale.php",{
method:"POST",
headers:{"Content-Type":"application/json"},
body:JSON.stringify({prompt:finalPrompt, topic:topic, brand:"selale", is_selale:true})
});
const text = await response.text();
let data;
try{ data = JSON.parse(text); }
...
try{
const response = await fetch("../api/brand_image_selale.php",{
method:"POST",
headers:{"Content-Type":"application/json"},
body:JSON.stringify({
...
if(subtitle === ""){
subtitle = "Aile, çocuk ve ebeveynlik konularında güvenilir ve bilgilendirici içerik.";
}
status.innerText = " Story görseli hazırlanıyor, lütfen bekleyin...";
try{
const response = await fetch("../api/brand_story_selale.php",{
method:"POST",
headers:{"Content-Type":"application/json"},
body:JSON.stringify({
...
function extractPlatformText(platform){
    ...
    function sectionBetween(text, starts, stops){
        ...
        for(const start of starts){
            const startRegex = new RegExp(start, "iu");
            const m = startRegex.exec(text);
            if(!m){ continue; }
            let part = text.slice(m.index + m[0].length);
            let cut = part.length;
            const allStops = stops.concat([
                "(?:^|\\n)\\s*[0-9]+\\s*[\\).:\\-]?\\s*[A-ZÇĞİÖŞÜa-zçğıöşü]+(?:\\s+[A-ZÇĞİÖŞÜa-zçğıöşü]+){0,4}\\s*:"
            ]);
            for(const stop of allStops){
                const stopRegex = new RegExp(stop, "iu");
                const sm = stopRegex.exec(part);
                if(sm && sm.index > 0 && sm.index < cut){ cut = sm.index; }
            }
    ...
    function cleanInstagramOnly(text){
        text = cleanText(text);
        if(text === ""){ return ""; }

        const cutPatterns = [
            "(?:^|\\n|\\r)\\s*2\\s*[\\).:\\-]?\\s*(?:INSTAGRAM\\s*)?(?:C[A-ZÇĞİÖŞÜa-zçğıöşü]{2,12}EL|KARUSEL|CAROUSEL|CARAUSEL|CARAUAEL|CARAOSEL|CARUSEL|CAROSEL)",
            "(?:^|\\n|\\r)\\s*(?:[0-9]+\\s*[\\).:\\-]?\\s*)?(?:INSTAGRAM\\s*)?(?:C[A-ZÇĞİÖŞÜa-zçğıöşü]{2,12}EL|KARUSEL|CAROUSEL|CARAUSEL|CARAUAEL|CARAOSEL|CARUSEL|CAROSEL)\\b",
            "(?:^|\\n|\\r)\\s*(?:[0-9]+\\s*[\\).:\\-]?\\s*)?(?:INSTAGRAM\\s*)?STORY\\b",
            "(?:^|\\n|\\r)\\s*(?:[0-9]+\\s*[\\).:\\-]?\\s*)?REELS?\\b",
            "(?:^|\\n|\\r)\\s*(?:[0-9]+\\s*[\\).:\\-]?\\s*)?FACEBOOK\\b",
            "(?:^|\\n|\\r)\\s*(?:[0-9]+\\s*[\\).:\\-]?\\s*)?LINKEDIN\\b",
            "(?:^|\\n|\\r)\\s*(?:[0-9]+\\s*[\\).:\\-]?\\s*)?THREADS\\b",
            "(?:^|\\n|\\r)\\s*(?:[0-9]+\\s*[\\).:\\-]?\\s*)?SHORTS\\b",
            "(?:^|\\n|\\r)\\s*(?:[0-9]+\\s*[\\).:\\-]?\\s*)?YOUTUBE\\b",
            "(?:^|\\n|\\r)\\s*(?:[0-9]+\\s*[\\).:\\-]?\\s*)?(?:INSTAGRAM\\s*)?G[ÖO]RSEL\\b"
        ];
        ...
        text = text.replace(/^\s*[0-9]+\s*[\).:\-]?\s*INSTAGRAM\s+(POSTU|POST|METNİ|METNI)\s*:?\s*/iu, "");
    ...
    function enrichIfTooShort(text, topic, platform){
        ...
        if(platform === "linkedin" && plain.length >= 700){ return text; }
        if(platform !== "linkedin" && plain.length >= 220){ return text; }
        let extra = (platform === "linkedin") ? topic + " konusunda aile içi iletişim, ilişkisel güven ve günlük yaşam düzeni birlikte değerlendirilmelidir. Sağlıklı değişim çoğu zaman tek bir öneriden değil; duyguyu anlamak, sınırları netleştirmek ve küçük ama sürdürülebilir adımlar atmaktan geçer." : topic + " konusunda en önemli nokta davranışın arkasındaki ihtiyacı görebilmektir. Küçük, tutarlı ve anlaşılır adımlar aile içi iletişimi güçlendirebilir.";
        return (text + "\n\n" + extra).trim();
    }
    ...
    if(platform === "instagram"){
        const ig = sectionBetween(content,
            ["(?:^|\\n)\\s*(?:1\\s*[\\).:\\-]?\\s*)?INSTAGRAM(?:\\s+POSTU|\\s+POST|\\s+METNİ|\\s+METNI)?\\s*:?"] ,
            ["(?:^|\\n)\\s*(?:2\\s*[\\).:\\-]?\\s*)?(?:INSTAGRAM\\s*)?(?:CAROUSEL|CARAUSEL|CARAUAEL|CARUSEL|CAROSEL|KARUSEL)", "(?:^|\\n)\\s*(?:3\\s*[\\).:\\-]?\\s*)?FACEBOOK", "(?:^|\\n)\\s*(?:4|5)\\s*[\\).:\\-]?\\s*LINKEDIN"]
        );
        return ensureHashtags(enrichIfTooShort(cleanInstagramOnly(firstGood(ig, social, topic)), topic, "instagram"), "#ailedanismanligi #aileiciiletisim #ebeveynlik #cocukgelisimi #duygusalfarkindalik");
    }

    if(platform === "facebook"){
        const fb = sectionBetween(content,
            ["(?:^|\\n)\\s*(?:3\\s*[\\).:\\-]?\\s*)?FACEBOOK(?:\\s+YAZISI|\\s+POSTU|\\s+POST|\\s+METNİ|\\s+METNI)?\\s*:?"] ,
            ["(?:^|\\n)\\s*(?:4|5)\\s*[\\).:\\-]?\\s*LINKEDIN", "(?:^|\\n)\\s*(?:2\\s*[\\).:\\-]?\\s*)?INSTAGRAM\\s*(?:CAROUSEL|CARAUSEL|CARAUAEL|CARUSEL|CAROSEL|KARUSEL)"]
        );
        return ensureHashtags(enrichIfTooShort(firstGood(fb, social, topic), topic, "facebook"), "#AileDanismanligi #AileIciIletisim #Ebeveynlik #CocukGelisimi #DuygusalFarkindalik");
    }

    if(platform === "linkedin"){
        const li = sectionBetween(content,
            ["(?:^|\\n)\\s*(?:4|5)\\s*[\\).:\\-]?\\s*LINKEDIN(?:\\s+YAZISI|\\s+POSTU|\\s+POST|\\s+METNİ|\\s+METNI)?\\s*:?"] ,
            ["(?:^|\\n)\\s*(?:5|6)\\s*[\\).:\\-]?\\s*INSTAGRAM\\s*STORY", "(?:^|\\n)\\s*(?:5|6)\\s*[\\).:\\-]?\\s*X\\b", "(?:^|\\n)\\s*(?:6|7)\\s*[\\).:\\-]?\\s*BLOG", "(?:^|\\n)\\s*THREADS", "(?:^|\\n)\\s*SHORTS", "(?:^|\\n)\\s*G[ÖO]RSEL"]
        );
        return ensureHashtags(enrichIfTooShort(firstGood(li, social, topic), topic, "linkedin"), "#AileDanismanligi #AileIciIletisim #Ebeveynlik #CocukGelisimi #DuygusalFarkindalik");
    }
