...
if($_SERVER["REQUEST_METHOD"] === "POST"){
    ...
    $brand = trim($_POST["brand"] ?? "ozgur");
    if(!in_array($brand, ["ozgur", "selale"], true)){
        $brand = "ozgur";
    }
    ...
    if($platform !== ""){
        $stmt = $pdo->prepare("
            ...
            (user_id, brand, platform, account_name, access_token, refresh_token, page_id, account_id, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, NOW())
        ");
        $stmt->execute([
...
.quick-threads{background:#000;}
.quick-selale{background:#7f1d1d;}
.quick-disabled{
    background:#94a3b8;
    cursor:not-allowed;
}
...
<div class="container">
    ...
    <div class="card">
        ...
        <div class="quick-connect">
            ...
            <a class="quick-btn quick-threads" href="../api/connect_threads.php?brand=ozgur">
                🧵 Threads Özgür
            </a>

            <a class="quick-btn quick-selale" href="../api/connect_threads.php?brand=selale">
                🧵 Threads Şelale
            </a>
        ...
        <div class="small">
            LinkedIn, Facebook ve Threads butonları OAuth bağlantısı açar. Threads için Özgür ve Şelale ayrı ayrı bağlanabilir.
        </div>
    </div>
