...
.platformBox label:has(input[value="bluesky"]){
background:linear-gradient(135deg,rgba(17,133,254,.78),rgba(56,189,248,.46));
}
...
<b></b>
<span>TOM aktif · İçerik, görsel, story, carousel, Threads, Bluesky ve yayın akışı hazır.</span>
</div>
...
<div class="heroStats">
    ...
    <div class="heroStat">
        <div class="heroStatIcon">📣</div>
        <span>Yayın</span>
        <strong>5 Kanal</strong>
        <small>FB · IG · LI · Threads · Bluesky</small>
    </div>
...
<label><input type="checkbox" class="publishPlatform" value="bluesky" checked style="width:auto;"> Bluesky</label>
<label><input type="checkbox" class="publishPlatform" value="threads" checked style="width:auto;"> Threads</label>
...
function extractPlatformText(platform){
    ...
    function enrichIfTooShort(text, topic, platform){
        ...
        let extra = (platform === "linkedin") ? topic + " ruh sağlığı okuryazarlığı açısından yalnızca bir belirti başlığı değil, kişinin işlevselliğini, ilişkilerini, uyku düzenini ve karar verme süreçlerini etkileyebilen bir alandır. Bu nedenle konuya popüler söylemlerle değil, bilimsel ve etik bir uzman bakışıyla yaklaşmak gerekir." : topic + " hakkında doğru bilgi, kişinin yaşadığı durumu daha sağlıklı anlamasına ve gerektiğinde uygun uzman desteğine yönelmesine yardımcı olabilir.";
        return (text + "\n\n" + extra).trim();
    }
  ...
  if(platform === "bluesky"){
    ...
    if(platform === "instagram"){
        ...
        return ensureHashtags(enrichIfTooShort(cleanInstagramOnly(firstGood(ig, social, topic)), topic, "instagram"), "#psikiyatri #ruhsagligi #duygusalfarkindalik #psikiyatribilgilendirme #ozgurozbebit");
    }
    ...
    if(platform === "facebook"){
        ...
        return ensureHashtags(enrichIfTooShort(firstGood(fb, social, topic), topic, "facebook"), "#Psikiyatri #RuhSagligi #DuygusalSaglik #PsikiyatriBilgilendirme #OzgurOzbebit");
    }
    ...
    if(platform === "linkedin"){
        ...
        return ensureHashtags(enrichIfTooShort(firstGood(li, social, topic), topic, "linkedin"), "#Psikiyatri #RuhSagligi #DuygusalSaglik #PsikiyatriBilgilendirme #OzgurOzbebit");
    }
...
return platform !== "threads" && platform !== "bluesky";
});

const hasThreads = selectedPlatforms.includes("threads");
const hasBluesky = selectedPlatforms.includes("bluesky");
...
bluesky: extractPlatformText("bluesky")
};
try{
    const microResponse = await fetch("../api/generate_micro_social_text.php", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
       body: JSON.stringify({
    topic: "Aşk beyinde nasıl çalışır?",
    content:
        platformTexts.instagram ||
        platformTexts.facebook ||
        platformTexts.linkedin ||
        platformTexts.threads ||
        platformTexts.bluesky ||
        "",
    platform: "bluesky"
})
    });

    const microText = await microResponse.text();
    let microData = {};

    try{
        microData = JSON.parse(microText);
        console.log("MICRO RAW:", microText);
console.log("MICRO DATA:", microData);
    }catch(e){
        console.warn("generate_micro_social_text.php JSON olmayan cevap verdi:", microText);
    }

   if(microData.success){
    const microFinalText =
        microData.text ||
        microData.texts?.bluesky ||
        microData.texts?.threads ||
        microData.texts?.x ||
        "";

    if(microFinalText){
        platformTexts.x = microData.texts?.x || microFinalText;
        platformTexts.threads = microData.texts?.threads || microFinalText;
        platformTexts.bluesky = microData.texts?.bluesky || microFinalText;
    }
}
    

}catch(e){
    console.warn("Mikro sosyal metin üretilemedi:", e);
}
...
if(hasBluesky){
    ...
    let blueskyText =
        platformTexts.bluesky ||
        document.getElementById("autoTopic").value.trim();

    blueskyText = String(blueskyText || "")
        .replace(/\s+/g, " ")
        .trim();

   if(blueskyText.length > 300){
    blueskyText = blueskyText.split(".")[0].trim() + ".";
}

    const response = await fetch("../api/publish_bluesky.php", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({
            text: blueskyText
    ...
    try{
        ...
        combinedResult.summary.push({
            platform:"bluesky",
            success:false,
            message:"publish_bluesky.php JSON olmayan cevap verdi: " + text.substring(0,500)
        });
        data = null;
    }
    ...
    if(data){
        combinedResult.summary.push({
            ...
            post_id:(data.result && (data.result.uri || data.result.cid)) ? (data.result.uri || data.result.cid) : ""
        });
...
{text:"Facebook / Instagram / LinkedIn / Threads / Bluesky sonuçları aşağıda", className:"wait"}
]);
