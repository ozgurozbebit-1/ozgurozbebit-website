...
function render(term) {
  ...
  const related = [...term.relatedTerms.map((link) => `<a href="/psikiyatri-sozlugu/${link.slug}/">${link.term}</a>`), ...term.relatedSiteLinks.map((link) => `<a href="${link.href}">${link.label}</a>`)].join("\n");
...
const selected = requested.length ? pilotTerms.filter((term) => requested.includes(term.slug)) : [];
const unknownSlugs = requested.filter((slug) => !selected.some((term) => term.slug === slug));
